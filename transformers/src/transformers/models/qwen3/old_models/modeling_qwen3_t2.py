...
#     """
#     Show exactly the last up-to-3 layers (3×1). Each subplot uses robust normalization.
#     """
...
#       - uncertainty/stop_unc_delta_{tag}.png         (NEW; Δ uncertainty, optional)
...
#     # confidence (full) — smooth outliers then plot
...
#             title="Δ Uncertainty (first difference)"
#         )
...
#     """
#     attn_stack: list[L] of (B,H,S,S) or (B,H,1,S).
#     → returns (B,L,S,S) with heads averaged and 'q' expanded to S (last query row if q=1).
#     """
#     if attn_stack is None or len(attn_stack) == 0:
#         print("Kir")
#         return None
...
#     # (B,S,S) → (B,bins), mean attention per |i-j| bin (CPU)
#     assert A_last.device.type == "cpu"
#     B, S, _ = A_last.shape
...
#     # CPU top-k per row → sparse → pooled
#     assert A_last.device.type == "cpu"
#     B, S, _ = A_last.shape
#     out = []
#     for r in ratios:
...
#     # CPU positional clusters → cluster×cluster map
#     assert A_last.device.type == "cpu"
#     B, S, _ = A_last.shape
#     out = []
#     for K in Ks:
...
#     plt.plot(x, II_list, label="I→I", linewidth=1.2)
#     plt.plot(x, IE_list, label="I→E", linewidth=1.0)
#     plt.plot(x, EI_list, label="E→I", linewidth=1.0)
...
#     """
#     3×1 panels: for each K, plot intra vs external vs layers.
#     """
...
#     """
#     3×1 panels: for each K, bar of last layer intra vs external.
#     """
#     L, S, _ = A_layers.shape
#     fig, axes = plt.subplots(1, len(Ks), figsize=(2.6*len(Ks), 2.0))
#     if len(Ks) == 1:
...
#     # Project token indices into this map’s axis length
#     S_tgt = M.shape[0]
...
# ):
#     """
#     Writes PNGs under {save_root}/attn_reductions/<method>/...  (small figs, 3×1 where applicable).
#     Shows </think> position(s) in all map-based plots (projected where needed). Unique filenames.
#     Also plots Internal vs External (span-based) and Grouped Intra/External (K∈{8,16,32}).
#     """
#     _ensure_dir(save_root)
...
#     # === Reductions (3×1 each where applicable) ===
#     try:
...
#     # robust fallback to bf16 if module has no params (unlikely)
#     for p in module.parameters():
#         return p.dtype
#     return torch.bfloat16

# def _num_groups(c: int, g: int = 8) -> int:
#     # pick a divisor of c, <= g
...
#     def __init__(self, d_in, d_out, pdrop=0.0):
#         super().__init__()
...
# def percentile(x: torch.Tensor, q: float, dim=None, keepdim=False):
#     # q in [0,1]
...
#     x:    (B,C,N)
#     mask: (B,1,N)  1=valid
#     returns (B,C,k) weighted by valid counts with area interpolation
#     """
#     B, C, N = x.shape
...
# # --------------------------------------------------------------------------------------
# # Feature Extractor: ATTENTION (B,L,H,N,N) -> z_att [B, D_ATT]
# # --------------------------------------------------------------------------------------

# class AttnFeatureExtractorLite(nn.Module):
#     """
#     Lite attention feature extractor that assumes input maps are already reduced
#     to a fixed k×k (done upstream in Qwen3Model). No downsampling/masking here.
...
#         # tiny 2D conv block shared across all (l,h) maps
#         self.conv1 = nn.Conv2d(1, c, 3, padding=1)
#         self.gn1   = nn.GroupNorm(max(1, min(8, c)), c)
#         self.conv2 = nn.Conv2d(c, c, 3, padding=1)
#         self.gn2   = nn.GroupNorm(max(1, min(8, c)), c)

#         # token projection head
#         self.proj = LinearLN(2 * c + 6, d_tok)  # 2c from avg/max pool; +6 stats

#         # positional embeddings for (layer, head)
#         self.layer_emb = nn.Embedding(max_layers, d_tok)
#         self.head_emb  = nn.Embedding(max_heads, d_tok)

#         # multi-query attentive pooling across tokens (L*H)
#         self.query = nn.Parameter(torch.randn(self.K, d_tok) * (1.0 / math.sqrt(d_tok)))
#         self.out   = nn.Sequential(
#             LinearLN(self.K * d_tok, D_ATT),
#             nn.Dropout(pdrop),
#         )
...
#         """
#         A: (B,1,k,k)  in fp32
#         returns (B,6)  [mean, var, entropy, diag_ratio, p90, sparsity]
#         """
#         # Flatten and base stats (all -> (B,1))
#         A2   = A.flatten(2)                      # (B,1,k*k)
#         mean = A2.mean(dim=-1)                   # (B,1)
#         var  = A2.var(dim=-1, unbiased=False)    # (B,1)

#         # Entropy of row-wise softmax, averaged over rows
#         rows = A.squeeze(1)                      # (B,k,k)
#         p    = F.softmax(rows, dim=-1).clamp_min(1e-8)
#         ent  = (-(p * p.log()).sum(dim=-1).mean(dim=-1)
#                ).unsqueeze(-1)                   # (B,1)

#         # Diagonal energy ratio: sum|diag| / sum|A|
#         diag = torch.diagonal(rows, dim1=-2, dim2=-1)          # (B,k)
#         diag_sum  = diag.abs().sum(dim=-1)                      # (B,)
#         rows_sum  = rows.abs().sum(dim=(-1, -2))                # (B,)
#         diag_ratio = (diag_sum / (rows_sum + 1e-6)).unsqueeze(-1)  # (B,1)

#         # High-percentile (p90) over all entries
#         p90  = percentile(A2, 0.90, dim=-1)                     # (B,1)

#         # Sparsity: share of entries below tiny relative threshold
#         A2_abs = A2.abs()                                       # (B,1,k*k)
...
#         x = maps_blhkk
#         x = self.gn1(F.gelu(self.conv1(x).to(torch.float32))).to(x.dtype)
#         x = self.gn2(F.gelu(self.conv2(x).to(torch.float32))).to(x.dtype)

#         gavg = F.adaptive_avg_pool2d(x, 1).flatten(1)  # (B*L*H, c)
#         gmax = F.adaptive_max_pool2d(x, 1).flatten(1)  # (B*L*H, c)
...
# # --------------------------------------------------------------------------------------
# # Feature Extractor: CONFIDENCE (B,N) -> z_conf [B, D_CONF]
# # --------------------------------------------------------------------------------------

# class ConfFeatureExtractorLite(nn.Module):
#     def __init__(self, D_CONF: int = 96, k_conf: int = 192, c: int = 32):
#         super().__init__()
...
#         self.proj  = LinearLN(2 * c + 5, D_CONF)
...
#         # Handle k==1 edge case (empty dx)
#         if dx.shape[-1] == 0:
#             mxs = torch.zeros_like(mean)             # (B,1)
#         else:
#             mxs = dx.abs().amax(dim=-1)              # (B,1)
...
#         """
#         conf: (B,N)
#         mask_tokens: (B,N) 1=valid
#         """
#         B, N = conf.shape
#         x = conf.detach().unsqueeze(1)  # (B,1,N)
#         if mask_tokens is None:
#             m = torch.ones(B, 1, N, device=conf.device, dtype=conf.dtype)
#         else:
...
#         h = self.gn1(F.gelu(self.conv1(x_ds).to(torch.float32))).to(x_ds.dtype)
#         h = self.gn2(F.gelu(self.conv2(h).to(torch.float32))).to(x_ds.dtype)

#         gavg = F.adaptive_avg_pool1d(h, 1).flatten(1)  # (B,c)
#         gmax = F.adaptive_max_pool1d(h, 1).flatten(1)  # (B,c)
...
# # --------------------------------------------------------------------------------------
# # Feature Extractor: LAST HIDDEN (B,N,D_model) -> z_hid [B, D_HID]
# # --------------------------------------------------------------------------------------

# class HiddenFeatureExtractorLite(nn.Module):
#     def __init__(self, D_model: int, D_HID: int = 192, k_hid: int = 192, d_tok: int = 192, groups: int = 8, pdrop: float = 0.10):
#         super().__init__()
...
#         # depthwise-ish temporal conv block
#         self.conv1 = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=min(groups, d_tok))
#         self.gn1   = nn.GroupNorm(_num_groups(d_tok), d_tok)
#         self.conv2 = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=min(groups, d_tok))
#         self.gn2   = nn.GroupNorm(_num_groups(d_tok), d_tok)

#         # 3 learned queries pooling over time
#         self.query = nn.Parameter(torch.randn(3, d_tok) * (1.0 / math.sqrt(d_tok)))

#         self.out = nn.Sequential(
#             LinearLN(3 * d_tok + 6, D_HID),
#             nn.Dropout(pdrop),
#         )
...
#         returns (B,6): mean, var, TV, p90|Δ|, early-late diff, last val
#         """
#         B, k = seq_mean.shape
#         mean = seq_mean.mean(dim=-1, keepdim=True)                     # (B,1)
#         var  = seq_mean.var(dim=-1, unbiased=False, keepdim=True)      # (B,1)

#         if k >= 2:
#             d    = seq_mean[:, 1:] - seq_mean[:, :-1]                  # (B,k-1)
#             tv   = d.abs().sum(dim=-1, keepdim=True)                   # (B,1)
#             p90d = percentile(d.abs(), 0.90, dim=-1, keepdim=True)     # (B,1)
#         else:
#             tv   = torch.zeros(B, 1, device=seq_mean.device, dtype=seq_mean.dtype)
#             p90d = torch.zeros_like(tv)

#         # early-late mean diff (use at least 1 token on each side)
#         w = max(1, k // 3)
#         el   = (seq_mean[:, :w].mean(dim=-1, keepdim=True)             # (B,1)
#                 - seq_mean[:, -w:].mean(dim=-1, keepdim=True))         # (B,1)

#         last = seq_mean[:, -1:].contiguous()                           # (B,1)

#         return torch.cat([mean, var, tv, p90d, el, last], dim=-1)      # (B,6)


#     def forward(self, last_hidden: torch.Tensor, mask_tokens: Optional[torch.Tensor] = None) -> torch.Tensor:
#         """
#         last_hidden: (B,N,D_model)
#         mask_tokens: (B,N) 1=valid
#         """
#         B, N, Dm = last_hidden.shape
#         x = self.proj(self.norm(last_hidden.detach()))  # (B,N,d_tok)
#         x = x.permute(0, 2, 1).contiguous()             # (B,d_tok,N)

#         if mask_tokens is None:
#             m = torch.ones(B, 1, N, device=last_hidden.device, dtype=last_hidden.dtype)
#         else:
...
#         tokens = h.permute(0, 2, 1).contiguous()  # (B,k,d_tok)
...
# # --------------------------------------------------------------------------------------
# # Correctness head (fusion): (z_att ⊕ z_conf ⊕ z_hid) -> (B,1)
# # --------------------------------------------------------------------------------------

# class CorrectnessHeadLite(nn.Module):
#     def __init__(self, D_ATT: int, D_CONF: int, D_HID: int, pdrop: float = 0.10):
#         super().__init__()
#         D = D_ATT + D_CONF + D_HID
...
#         return self.mlp(torch.cat([z_att, z_conf, z_hid], dim=-1))  # (B,1)

# # --------------------------------------------------------------------------------------
# # Your model, refactored to return (B,1)
# # --------------------------------------------------------------------------------------
...
#         # ─── main layer loop ────────────────────────────────────────────────────
#         for idx, decoder_layer in enumerate(self.layers):
#             need_attn = output_attentions and (idx % 5 == 9)
...
# def _save_attn_layer_grid(A_l_hkk: np.ndarray, save_path: str, heads_per_row: int = 8, title: str = None):
#     """
#     A_l_hkk: (H,k,k) numpy float32
#     Saves a grid image with all H heads for one layer.
#     """
#     import matplotlib
#     matplotlib.use("Agg")
#     import matplotlib.pyplot as plt
...
#     H, k, _ = A_l_hkk.shape
#     cols = min(heads_per_row, H)
#     rows = int(math.ceil(H / float(cols)))
#     fig_w = 2.0 * cols
#     fig_h = 2.0 * rows
#     fig, axes = plt.subplots(rows, cols, figsize=(fig_w, fig_h), squeeze=False)
#     vmax = float(np.abs(A_l_hkk).max()) or 1.0

#     for h in range(H):
#         r, c = divmod(h, cols)
#         ax = axes[r][c]
#         ax.imshow(A_l_hkk[h], origin="lower", interpolation="nearest",
...
#     plt.tight_layout(rect=[0, 0.00, 1, 0.95] if title else [0,0,1,1])
#     fig.savefig(save_path, dpi=160, bbox_inches="tight")
#     plt.close(fig)

# def save_attn_stack_grids(A_blhkk: torch.Tensor, out_dir: str, tag: str = "", max_layers: int = None, heads_per_row: int = 8):
#     """
#     A_blhkk: (B,L,H,k,k) reduced attention stack (input to the extractor)
#     Saves one PNG per layer for batch item 0.
#     """
#     if out_dir is None:
#         return
#     _ensure_dir(out_dir)
#     with torch.no_grad():
#         if A_blhkk is None or A_blhkk.numel() == 0:
#             return
#         A = A_blhkk[0]  # visualize first batch only to keep it light  -> (L,H,k,k)
#         L, H, k, _ = A.shape
#         Lp = L if (max_layers is None) else min(L, max_layers)
#         A = A[:Lp].to(torch.float32).cpu().numpy()
#         for l in range(Lp):
#             save_path = os.path.join(out_dir, f"{tag}layer{l:02d}.png")
#             _save_attn_layer_grid(A[l], save_path, heads_per_row=heads_per_row,
#                                   title=f"Layer {l} • H={H} • k={k}")
...
#         # ---- dims (with safe defaults) ----
#         D_ATT  = int(getattr(config, "stop_att_dim", 384))
#         D_CONF = int(getattr(config, "stop_conf_dim", 96))
#         D_HID  = int(getattr(config, "stop_hid_dim", 192))

#         k_att  = int(getattr(config, "stop_k_att", 192))
...
#         max_heads  = config.num_attention_heads
...
#         self.conf_extractor = ConfFeatureExtractorLite(D_CONF=D_CONF, k_conf=k_conf, c=32)
#         self.hid_extractor  = HiddenFeatureExtractorLite(D_model=config.hidden_size, D_HID=D_HID, k_hid=k_hid, d_tok=192, groups=8, pdrop=0.10)
#         self.stop_head      = CorrectnessHeadLite(D_ATT=D_ATT, D_CONF=D_CONF, D_HID=D_HID, pdrop=0.10)
...
#         # init & freeze base (train only aux modules)
#         self.post_init()
#         for n, p in self.named_parameters():
#             if not (n.startswith("attn_extractor") or n.startswith("conf_extractor") or
#                     n.startswith("hid_extractor") or n.startswith("stop_head")):
#                 p.requires_grad_(False)

#     # ----------------------------------------------------------------------------------
#     # Aux correctness scorer
#     # ----------------------------------------------------------------------------------
#     # def _should_stop(
#     #     self,
#     #     last_hidden: torch.Tensor,                 # (B,S,Hd)
#     #     attn_stack: Optional[List[torch.Tensor]],  # length L: each is (B,H,S,S)
#     #     token_probs: torch.Tensor,                 # (B,S)
...
#     #     # stack attention (detached)
#     #     if attn_stack is not None and len(attn_stack) > 0:
#     #         with torch.no_grad():
#     #             A = torch.stack(attn_stack, dim=1).detach()  # (B,L,H,S,S)
#     #     else:
#     #         A = None


#     #     # # --- optional plotting (input to extractor) ---
#     #     # # Priority: explicit plot_dir arg; else self.attn_plot_dir if defined.
#     #     # _plot_dir = "/home/amirhosein/codes/SelfAwareMachine/viss"
#     #     # if _plot_dir and (A is not None):
#     #     #     try:
#     #     #         # tag example: global step or a simple monotonic counter if you keep one
#     #     #         tag = getattr(self, "attn_plot_tag", "")
#     #     #         save_attn_stack_grids(A, _plot_dir, tag=("" if tag == "" else f"{tag}_"))
#     #     #     except Exception as e:
#     #     #         # Never let plotting break training
#     #     #         if getattr(self, "verbose_attn_plot_errors", False):
#     #     #             print(f"[attn-plot] failed: {e}")
...
#     #         else:
#     #             D_ATT = getattr(self.attn_extractor.out[0].fc, "out_features", 384)
#     #             z_att = torch.zeros(B, D_ATT, device=last_hidden.device, dtype=out_dtype)

#     #         # ---- extract z_conf ----
#     #         z_conf = self.conf_extractor(token_probs.to(_safe_dtype_param(self.conf_extractor)), mask_tokens=mask)

#     #         # ---- extract z_hid ----
#     #         z_hid  = self.hid_extractor(last_hidden.to(_safe_dtype_param(self.hid_extractor)), mask_tokens=mask)

#     #         logits = self.stop_head(z_att.to(out_dtype), z_conf.to(out_dtype), z_hid.to(out_dtype))  # (B,1)
...
#         # ---------- stack & validate reduced attention ----------
#         if attn_stack is not None and len(attn_stack) > 0:
#             with torch.no_grad():
#                 A = torch.stack(attn_stack, dim=1).detach()  # (B,L,H,k,k)
#                 # fail-fast if shape is not reduced & square
...
#         # ---------- sanitize other inputs early ----------
#         token_probs_s = torch.nan_to_num(token_probs, nan=1.0, posinf=1.0, neginf=1e-8)
#         token_probs_s = torch.clamp(token_probs_s, 1e-8, 1.0)
...
#                     z_att = self.attn_extractor(A.to(_safe_dtype_param(self.attn_extractor)), mask_tokens=mask)
#                 except Exception as e:
#                     if not hasattr(self, "_warned_attn_extractor"):
#                         print(f"[warn] attn_extractor failed ({e}); using zeros thereafter.")
#                         self._warned_attn_extractor = True
#                     D_ATT = getattr(self.attn_extractor.out[0].fc, "out_features", 384)
#                     z_att = torch.zeros(B, D_ATT, device=last_hidden.device, dtype=out_dtype)
#             else:
#                 D_ATT = getattr(self.attn_extractor.out[0].fc, "out_features", 384)
...
#             # z_conf / z_hid
#             z_conf = self.conf_extractor(token_probs_s.to(_safe_dtype_param(self.conf_extractor)), mask_tokens=mask)
#             z_hid  = self.hid_extractor(last_hidden_s.to(_safe_dtype_param(self.hid_extractor)), mask_tokens=mask)

#             # sanitize features (keep grads; nan_to_num is differentiable)
#             z_att  = torch.nan_to_num(z_att,  nan=0.0, posinf=0.0, neginf=0.0)
#             z_conf = torch.nan_to_num(z_conf, nan=0.0, posinf=0.0, neginf=0.0)
#             z_hid  = torch.nan_to_num(z_hid,  nan=0.0, posinf=0.0, neginf=0.0)

#             logits = self.stop_head(
#                 z_att.to(out_dtype), z_conf.to(out_dtype), z_hid.to(out_dtype)
#             )  # (B,1)
...
#         probs = torch.clamp(probs, 1e-6, 1.0 - 1e-6).to(out_dtype)
#         return probs  # (B,1)
...
#         #  ↓↓↓ extra kwargs used at generation
...
#                     # log-softmax in full-precision → avoids under-flow
...
#                     # ----- optional: return to prob-space BUT clamp to ε so they’re never tiny -----
#                     eps = 1e-8
