...
#         # projection to tokens (avg/max + stats6 → d_tok)
...
#       - 3×Conv1d (GN+GELU) with residual + SE1d
...
#       - 2×Conv1d (depthwise-ish via groups) + residual + SE1d
#       - optional tiny token mixer across time (disabled by default)
#       - 3 learnable queries pool → pooled vector
#       - add fp32 dynamic stats6 → LinearLN to D_HID
#     """
#     def __init__(
#         self,
#         D_model: int,
...
#             nn.Linear(128, 1),
#         )
#     def forward(self, z_att: torch.Tensor, z_conf: torch.Tensor, z_hid: torch.Tensor) -> torch.Tensor:
#         x = torch.cat([z_att, z_conf, z_hid], dim=-1)
...
# __all__ = ["Qwen3PreTrainedModel", "Qwen3Model", "Qwen3ForCausalLM"]
...
#         D_CONF = int(getattr(config, "stop_conf_dim", 128))  # slightly larger (more head capacity)
#         D_HID  = int(getattr(config, "stop_hid_dim", 256))   # slightly larger (more head capacity)
...
#         # ---- stronger-but-compact aux modules (from your extractors/head) ----
...
#         # expose for init/freeze utilities
#         self._custom_head_names = ["attn_extractor", "conf_extractor", "hid_extractor", "stop_head"]

#         # init weights -> will be properly (re)initialized by your post-load routine
#         self.post_init()

#         # freeze base; train only aux modules
#         for n, p in self.named_parameters():
#             if not (n.startswith("attn_extractor") or n.startswith("conf_extractor")
#                     or n.startswith("hid_extractor") or n.startswith("stop_head")):
...
#                     D_ATT = getattr(self.stop_head.net[0], "normalized_shape", [384])[0]
