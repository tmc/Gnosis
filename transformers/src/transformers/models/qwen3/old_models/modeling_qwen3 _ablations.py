...
#         # # Only spectral radial bands (5-D)
#         # stats_groups=("spec_bands5",)

#         # # Only row/col dispersion & entropies (4-D)
#         # stats_groups=("rowcol4",)

#         # # Only structural cues (3-D)
#         # stats_groups=("structure3",)

#         # # Only Laplacian trace (1-D)
#         # stats_groups=("lap1",)

#         # # Moments only (6-D)
#         # stats_groups=("moments6",)

#         # # Global entropies & MI only (4-D)
#         # stats_groups=("entropy4",)

#         # # Radial bands + moments (11-D)
#         # stats_groups=("spec_bands5", "moments6")

#         # # Legacy 13 + moments (19-D)
#         # stats_groups=("spec13", "moments6")

#         # # Everything (23-D)
#         # stats_groups=("all",)
#         ATTN_VARIANT = "d3"  # choose: "lite", "d2", or "d3"
#         if self.use_stop_attn:
#             if ATTN_VARIANT == "lite":
...
#             elif ATTN_VARIANT == "d2":
#                 self.attn_extractor = AttnFeatureExtractorLite_D2(
#                     D_ATT=D_ATT,
#                     d_grid=128,
...
#                 # D3, spectral-only (no CNN modules constructed)
#                 self.attn_extractor = AttnFeatureExtractorLite_D3(
#                     D_ATT=D_ATT,
#                     d_grid=192,
#                     cnn_channels=(32, 64, 128),
...
#                     max_layers=max_layers,
#                     max_heads=max_heads,
#                     feature_mode="spectral",              # <-- CNN disabled & not initialized
...
#             # elif ATTN_VARIANT == "d4_gpt": #4B
#             #     self.attn_extractor = AttnFeatureExtractorLite_D4_gpt(
#             #         D_ATT=D_ATT,
#             #         d_tok=192,
#             #         cnn_channels=(64, 128, 192),
#             #         axial_blocks=3,
#             #         axial_heads=4,
#             #         axial_drop_path=0.10,
#             #         tcn_dilations=(1, 2, 4),
#             #         K=6,
#             #         pdrop=0.10,
#             #         max_layers=max_layers,
#             #         max_heads=max_heads,
#             #     )
#             # elif ATTN_VARIANT == "d4_gpt_nopos": #4B
#             #     self.attn_extractor = AttnFeatureExtractorLite_D4_gpt_NoPos(
#             #         D_ATT=D_ATT,
#             #         d_tok=192,
#             #         cnn_channels=(64, 128, 192),
#             #         axial_blocks=3,
...
#             #         max_heads=max_heads,
#             #     )
#             # elif ATTN_VARIANT == "d4_gemini":
#             #     # D4, spectral-only (no CNN modules constructed)
#             #     self.attn_extractor = AttnFeatureExtractorLite_D4_gemini(
#             #         D_ATT=D_ATT,
#             #         d_grid=192,                     # must be divisible by grid_transformer_heads (192 % 6 == 0)
#             #         cnn_channels=(32, 64, 128),
#             #         grid_transformer_layers=12,
#             #         grid_transformer_heads=6,
#             #         pdrop=0.10,
#             #         max_layers=max_layers,
#             #         max_heads=max_heads,
#             #         feature_mode="spectral",        # <-- disables CNN & avoids allocating its params
#             #         stats_groups=("all",),       # <-- same ablation knob as D3
#             #         spec_radii=(0.15, 0.35, 0.60),  # optional: ring cutoffs
#             #         band_widths=(None, None),       # optional: defaults to (k//32, k//16)
#             #     )
#             # elif ATTN_VARIANT == "d5":
#             #     self.attn_extractor = AttnFeatureExtractor_D5(
#             #         D_ATT=D_ATT,
#             #         d_grid=192,                  # token dim over (L,H)
#             #         cnn_channels=(48, 96, 192),  # per-map CNN pyramid channels (256→128→64→32)
#             #         grid_conv_layers=3,          # axial conv mixing depth
#             #         K=6,                         # PMA seeds
#             #         pdrop=0.10,
#             #         # axial Transformer refiner (pure content-only: no pos enc, no distance bias)
#             #         sa_blocks=3,                 # set 0 to disable transformers (pure D3)
#             #         sa_heads=4,
#             #         sa_attn_drop=0.10,
#             #         sa_drop_path=0.10,
#             #     )
...
#                 #     D_model=config.hidden_size,
#                 #     D_HID=D_HID,
#                 #     d_tok=192*2,          # token dim over time
#                 #     k_hid=k_hid,        # downsampled time steps
#                 #     groups=8,           # depthwise grouping for 1D convs
#                 #     K=8,                # PMA seeds
#                 #     sab_layers=3,
...
#             # elif ATTN_VARIANT == "V5":
#             #     # Upgraded extractor with SDPA SAB/PMA, DropPath, multi-scale pooling, richer stats
#             #     # Requires: HiddenFeatureExtractorLiteV5 to be defined/imported.
#             #     self.hid_extractor = HiddenFeatureExtractorLite_V5(
#             #         D_model = config.hidden_size,
#             #         D_HID   = int(D_HID),
#             #         d_tok   = 192,
#             #         k_hid   = int(k_hid),
...
#             #     )
#             # if ATTN_VARIANT == "V6":
#                 self.hid_extractor = HiddenFeatureExtractorLite_V6(
#                     d_llm=config.hidden_size,  # Input LLM dim
#                     D_HID=D_HID,               # Final output feature dim
#                     d_model=192,               # Internal model dim (matches V1 d_tok)
#                     n_heads=4,                 # (matches V1 sab_heads)
#                     num_layers=3,              # (matches V1 sab_layers)
#                     d_ff=192 * 4,              # Standard FFN expansion
#                     dropout=0.10,
#                 )
#             # if ATTN_VARIANT == "V2":
#             #     #1m
#             #     self.hid_extractor = HiddenFeatureExtractorLite_V2(
#             #         D_model = config.hidden_size,
#             #         D_HID   = D_HID,
#             #         d_tok   = 192,   # token channel dim
#             #         k_hid   = k_hid, # total downsampled steps
#             #         k_tail  = 64,
#             #         pdrop   = 0.10,
#             #         last_m  = 16,    # avg window over last tokens
#             #         conv_k  = 5,     # TCN kernel size
#             #     )
#             # if ATTN_VARIANT == "V3":
#             #     self.hid_extractor = HiddenFeatureExtractorLite_V3(
#             #         D_model        = config.hidden_size,
#             #         D_HID          = D_HID,
#             #         d_tok          = 192,     # wider token channels
#             #         k_hid          = k_hid,
#             #         k_tail         = 64,  # raw tail tokens kept
#             #         pdrop          = 0.10,
#             #         last_m         = 24,      # tail-average window
#             #         conv_k         = 5,       # wider conv kernel
#             #         expand         = 4,       # TCN expansion ratio
#             #         drop_path      = 0.10,    # stochastic depth across TCN stack
#             #         attn_layers    = 4,       # tiny Transformer mixer (set 0 to disable)
#             #         attn_heads     = 8,
#             #         attn_ff_mult   = 4,
#             #         use_pos_embed  = True,
#             #     )
#             # if ATTN_VARIANT == "V4":
#                 # self.hid_extractor = HiddenFeatureExtractorLite_V4(
#                 #     D_model       = config.hidden_size,
#                 #     D_HID         = D_HID,   # downstream head size
#                 #     d_tok         = 192,     # 512 for even more capacity
#                 #     k_hid         = k_hid,   # e.g., 256
#                 #     k_tail        = 64,  # e.g., 96
#                 #     pdrop         = 0.10,
#                 #     conv_k        = 5,
#                 #     tcn_expand    = 2,
#                 #     tcn_drop_path = 0.10,
#                 #     tf_layers     = 4,       # set 0 to disable Transformer mixer
#                 #     tf_heads      = 8,
#                 #     tf_ff_mult    = 2,
#                 #     seeds_K       = 6,
#                 #     seeds_heads   = 6,
...
#         # # Or V4: Product-of-Experts
#         self.stop_head = CorrectnessHead_V4(
#             D_ATT=256, D_CONF=128, D_HID=256,
#             use_attn=True, use_conf=False, use_hid=True,
#             D_FUSE=256, pdrop=0.10, min_tau=0.2, max_tau=5.0
#         )
...
#         # self.stop_head = CorrectnessHead_V2(
#         #     D_ATT=D_ATT,
#         #     D_CONF=D_CONF,
#         #     D_HID=D_HID,
#         #     use_attn=True,
...
#         #     n_layers=2,
#         #     pdrop=P_DROP,
#         #     modality_drop_p=MODALITY_DROP_P,
#         #     use_temperature=True,
#         # )

#         # self.stop_head = CorrectnessHead_V3(
#         #     D_ATT=D_ATT,
#         #     D_CONF=D_CONF,
#         #     D_HID=D_HID,
#         #     use_attn=True,
