...
#     """
#     B, C, N = x.shape
#     # assert mask.shape == (B, 1, N), f"mask shape must be {mask.shape}(B,1,N), got {mask.shape}"
...
# # ---- tiny residual MLP used by the head ----
# class _ResidualMLP(nn.Module):
#     def __init__(self, width: int, pdrop: float = 0.10):
#         super().__init__()
...
# # (deeper conv stem + SE, two-stage projection, query pooling)
# # ======================================================
# class AttnFeatureExtractorLite(nn.Module):
#     """
#     Simple-strong redesign:
#       - Conv2d(1->c) + 3 residual conv blocks (c const) + SE2d
#       - Global avg/max + fp32 stats6
#       - 2-stage LinearLN to d_tok
#       - (layer,head) positional emb
#       - K-query attentive pooling -> LinearLN to D_ATT
#     """
#     def __init__(
#         self,
#         D_ATT: int = 384,
...
#         max_heads: int = 128,
#         pdrop: float = 0.10,
#         mixer_layers: int = 1,           # accepted for API compatibility (unused)
#         mixer_heads: int = 4,
#         mixer_enable_T_max: int = 2048,
#     ):
#         super().__init__()
#         self.c = int(c)
...
#         self.gn_in   = nn.GroupNorm(_num_groups(c), c)

#         # 3 residual conv blocks (simple, constant width)
#         self.conv1a = nn.Conv2d(c, c, 3, padding=1); self.gn1a = nn.GroupNorm(_num_groups(c), c)
#         self.conv1b = nn.Conv2d(c, c, 3, padding=1); self.gn1b = nn.GroupNorm(_num_groups(c), c)

#         self.conv2a = nn.Conv2d(c, c, 3, padding=1); self.gn2a = nn.GroupNorm(_num_groups(c), c)
#         self.conv2b = nn.Conv2d(c, c, 3, padding=1); self.gn2b = nn.GroupNorm(_num_groups(c), c)

#         self.conv3a = nn.Conv2d(c, c, 3, padding=1); self.gn3a = nn.GroupNorm(_num_groups(c), c)
#         self.conv3b = nn.Conv2d(c, c, 3, padding=1); self.gn3b = nn.GroupNorm(_num_groups(c), c)

#         self.se2d   = SE2d(c, r=8)
#         self.drop   = nn.Dropout(pdrop)

#         # 2-stage projection to token dim
#         self.proj = nn.Sequential(
#             LinearLN(2 * c + 6, 2 * d_tok, pdrop=pdrop),
#             LinearLN(2 * d_tok, d_tok,     pdrop=pdrop * 0.5),
#         )
...
#         mean = A2.mean(dim=-1)
#         var  = A2.var(dim=-1, unbiased=False)
#         rows = A.squeeze(1)                    # (B,k,k)
#         p    = F.softmax(rows, dim=-1).clamp_min(1e-8)
#         ent  = (-(p * p.log()).sum(dim=-1).mean(dim=-1)).unsqueeze(-1)
#         diag = torch.diagonal(rows, dim1=-2, dim2=-1)
#         diag_ratio = (diag.abs().sum(dim=-1) / (rows.abs().sum(dim=(-1, -2)) + 1e-6)).unsqueeze(-1)
#         p90  = percentile(A2, 0.90, dim=-1)
#         A2_abs = A2.abs()
#         thr    = (A2_abs.mean(dim=-1) * 1e-3)
#         sparse = (A2_abs < thr.unsqueeze(-1)).float().mean(dim=-1)
#         return torch.cat([mean, var, ent, diag_ratio, p90, sparse], dim=-1)  # (B,6)

#     def _res_block2d(self, x, convA, gnA, convB, gnB):
#         y = F.gelu(gnA(convA(x).to(torch.float32))).to(x.dtype)
#         y = F.gelu(gnB(convB(y).to(torch.float32))).to(x.dtype)
...
#         x  = F.gelu(self.gn_in(self.conv_in(maps_blhkk).to(torch.float32))).to(maps_blhkk.dtype)
#         x  = self._res_block2d(x, self.conv1a, self.gn1a, self.conv1b, self.gn1b)
#         x  = self._res_block2d(x, self.conv2a, self.gn2a, self.conv2b, self.gn2b)
#         x  = self._res_block2d(x, self.conv3a, self.gn3a, self.conv3b, self.gn3b)
#         x  = self.se2d(x)

#         gavg = F.adaptive_avg_pool2d(x, 1).flatten(1)
#         gmax = F.adaptive_max_pool2d(x, 1).flatten(1)
...
#         pe = (self.layer_emb(l_idx.to(device)) + self.head_emb(h_idx.to(device)))  # (T,d_tok)
#         toks = tok.view(B, T, self.d_tok) + pe.unsqueeze(0)                        # (B,T,d_tok)
#         return toks

#     def forward(self, attn: torch.Tensor, mask_tokens: torch.Tensor = None) -> torch.Tensor:
...
#         scores = torch.einsum('btd,kd->bkt', toks, q) / math.sqrt(self.d_tok)
...
# # (mask-aware ds, deeper 1D conv stem + SE, two-stage projection)
# # ==================================================
# class ConfFeatureExtractorLite(nn.Module):
#     """
#     Simple-strong redesign:
#       - mask-aware area downsample to k_conf
#       - Conv1d(1->c) + 2 residual conv blocks (c const) + SE1d
#       - Global avg/max + fp32 stats5
#       - 2-stage LinearLN to D_CONF
#     """
...
#         self.conv_in = nn.Conv1d(1, c, 5, padding=2)
#         self.gn_in   = nn.GroupNorm(_num_groups(c), c)

#         self.conv1a = nn.Conv1d(c, c, 5, padding=2); self.gn1a = nn.GroupNorm(_num_groups(c), c)
#         self.conv1b = nn.Conv1d(c, c, 5, padding=2); self.gn1b = nn.GroupNorm(_num_groups(c), c)

#         self.conv2a = nn.Conv1d(c, c, 5, padding=2); self.gn2a = nn.GroupNorm(_num_groups(c), c)
#         self.conv2b = nn.Conv1d(c, c, 5, padding=2); self.gn2b = nn.GroupNorm(_num_groups(c), c)

#         self.se1d   = SE1d(c, r=8)
#         self.drop   = nn.Dropout(pdrop)

#         self.proj  = nn.Sequential(
#             LinearLN(2 * c + 5, 2 * D_CONF, pdrop=pdrop),
#             LinearLN(2 * D_CONF, D_CONF,   pdrop=pdrop * 0.5),
#         )
...
#         p90  = percentile(x, 0.90, dim=-1)
#         return torch.cat([mean, var, tv, mxs, p90], dim=-1)

#     def _res_block1d(self, x, convA, gnA, convB, gnB):
#         y = F.gelu(gnA(convA(x).to(torch.float32))).to(x.dtype)
#         y = F.gelu(gnB(convB(y).to(torch.float32))).to(x.dtype)
#         return self.drop(x + y)

#     def forward(self, conf: torch.Tensor, mask_tokens: torch.Tensor = None) -> torch.Tensor:
#         B, S = conf.shape
...
#         y = F.gelu(self.gn_in(self.conv_in(x_ds).to(torch.float32))).to(x_ds.dtype)
#         y = self._res_block1d(y, self.conv1a, self.gn1a, self.conv1b, self.gn1b)
#         y = self._res_block1d(y, self.conv2a, self.gn2a, self.conv2b, self.gn2b)
#         y = self.se1d(y)

#         gavg = F.adaptive_avg_pool1d(y, 1).flatten(1)
#         gmax = F.adaptive_max_pool1d(y, 1).flatten(1)

#         with autocast("cuda", enabled=False):
#             stats = self._stats5(x_ds.to(torch.float32))
...
# # (LN+Linear, mask-aware ds, deeper grouped convs + SE, query pooling)
# # ==========================================================
...
#       - 3 residual grouped Conv1d blocks (d_tok const) + SE1d
#       - 3 learnable queries over time
#       - fp32 dynamic stats6 on mean trajectory
#       - LinearLN to D_HID
#     """
#     def __init__(
#         self,
...
#         groups: int = 8,
#         pdrop: float = 0.10,
#         mixer_layers: int = 0,        # accepted for API compatibility (unused)
#         mixer_heads: int = 4,
...
#         g = min(max(2, groups), d_tok)
#         # 3 residual grouped conv blocks
#         self.conv1a = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=g); self.gn1a = nn.GroupNorm(_num_groups(d_tok), d_tok)
#         self.conv1b = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=g); self.gn1b = nn.GroupNorm(_num_groups(d_tok), d_tok)

#         self.conv2a = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=g); self.gn2a = nn.GroupNorm(_num_groups(d_tok), d_tok)
#         self.conv2b = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=g); self.gn2b = nn.GroupNorm(_num_groups(d_tok), d_tok)

#         self.conv3a = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=g); self.gn3a = nn.GroupNorm(_num_groups(d_tok), d_tok)
#         self.conv3b = nn.Conv1d(d_tok, d_tok, 5, padding=2, groups=g); self.gn3b = nn.GroupNorm(_num_groups(d_tok), d_tok)

#         self.se1d  = SE1d(d_tok, r=8)
#         self.drop  = nn.Dropout(pdrop)

#         # 3 learnable queries (match original)
...
#     def forward(self, last_hidden: torch.Tensor, mask_tokens: torch.Tensor = None) -> torch.Tensor:
...
#         y = self._res_block1d(x_ds, self.conv1a, self.gn1a, self.conv1b, self.gn1b)
#         y = self._res_block1d(y,    self.conv2a, self.gn2a, self.conv2b, self.gn2b)
#         y = self._res_block1d(y,    self.conv3a, self.gn3a, self.conv3b, self.gn3b)
#         y = self.se1d(y)

#         tokens = y.permute(0, 2, 1).contiguous()  # (B,k,d_tok)
...
# # (wider MLP with residual, same API)
# # =========================
# class CorrectnessHeadLite(nn.Module):
#     """
#     Stronger MLP head with PreNorm + residual blocks.
#     Requires all enabled feature vectors to be provided (no zero-filling).
#     """
#     def __init__(
#         self,
#         D_ATT: int,
...
#         self.in_proj = nn.Linear(D, 512)
#         self.block1  = _ResidualMLP(512, pdrop=pdrop)
#         self.block2  = _ResidualMLP(512, pdrop=pdrop)
#         self.out     = nn.Sequential(
#             nn.LayerNorm(512),
#             nn.GELU(),
#             nn.Dropout(pdrop),
...
#     def forward(
#         self,
#         z_att: torch.Tensor = None,
#         z_conf: torch.Tensor = None,
#         z_hid: torch.Tensor = None,
#     ) -> torch.Tensor:
#         parts = []
#         if self.use_attn:
#             if z_att is None:
...
#     # (rest of your class — forward(), compute_stop_loss(), get/set embeddings, etc. — unchanged)
