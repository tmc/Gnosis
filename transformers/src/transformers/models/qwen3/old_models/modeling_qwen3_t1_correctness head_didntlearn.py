...
#             in_ch = hidden
#             dilation *= 2           # double dilation → exponential RF

#         # final 1×1 projection to out_dim
...
#         """
#         probs : (B, S)  – token probabilities (any float dtype)
...
#     size: int,             # target k (e.g., 16/32/64/…)
#     sigma: float = 1.0
# ) -> torch.Tensor:
#     """
...
# # def gauss_blur_adaptive_pool(  # kept the name for backward-compat
# #     A_tok: torch.Tensor,   # (B, C, S, S)
# #     size: int,             # target k (e.g., 16/32/64/…)
# #     sigma: float = 1.0     # unused; kept for signature compatibility
# # ) -> torch.Tensor:
# #     """
# #     Strided average pooling down to (size, size). No Gaussian blur.
# #     Exact output shape (B, C, size, size), differentiable.

# #     For k > 1, choose stride s = floor((S-1)/(k-1)) and kernel K = S - s*(k-1),
# #     which ensures out = floor((S - K)/s) + 1 = k exactly (no padding).
# #     Also chunks across channels if B*C*S*S is too large for 32-bit index math.
# #     """
# #     assert A_tok.dim() == 4, "A_tok must be (B,C,S,S)"
# #     B, C, S, T = A_tok.shape
# #     assert S == T, "attention maps must be square"
...
# #         return A_tok
# #     if k > S:
# #         raise ValueError("Strided pooling is a downsampling op; require size <= input side length.")

# #     # k == 1 is a simple global average
# #     if k == 1:
# #         return F.avg_pool2d(A_tok, kernel_size=S, stride=S)

# #     # Compute stride and kernel to get EXACT k outputs with no padding.
# #     s = max(1, (S - 1) // (k - 1))          # stride
# #     K = S - s * (k - 1)                      # kernel size (guaranteed >= 1)

# #     # Guard against 32-bit index math limits by chunking channels if needed
# #     MAX_32 = 2**31 - 1
# #     elems_per_channel = B * S * S
# #     # max channels per chunk so that input.numel() <= MAX_32
# #     c_per_chunk = max(1, min(C, MAX_32 // max(1, elems_per_channel)))

# #     if c_per_chunk < C:
# #         outs = []
# #         for i in range(0, C, c_per_chunk):
# #             chunk = A_tok[:, i:i + c_per_chunk]
# #             outs.append(F.avg_pool2d(chunk, kernel_size=K, stride=s))
# #         return torch.cat(outs, dim=1)

# #     return F.avg_pool2d(A_tok, kernel_size=K, stride=s)
...
#                 A = attn_full.mean(dim=1)

#                 # (2) reduce to k×k; use your helper if available
#                 A = A.unsqueeze(1)  # (B,1,S,S)
#                 A_small = gauss_blur_adaptive_pool(A, size=128, sigma=1).squeeze(1)  # (B,k,k)
...
#         # COLLECT_EVERY = 8                       # we keep layers 3,7,11,…
...
#             if not (n.startswith("hid_encoder") or  n.startswith("axial_sent_encoder") or n.startswith("conf_encoder") or n.startswith("stop_head")):
...
#         attn_stack:  Optional[List[torch.Tensor]],   # L × (B,H,S,S)  or None
#         token_probs: torch.Tensor,          # (B,S)
#         mask:        torch.Tensor,          # (B,S)
...
#         # ── 1) Hidden-state encoder  (B,S,D_HID) ─────────────────────────────
#         z_hid = self.hid_encoder(last_hidden.to(hid_dtype))          # (B,S,D_HID)

#         A_sent = torch.stack(attn_stack, dim=1).to(ax_dtype)          # contiguous
#         # print(A_sent)
#         # ── 3) Sentence-level axial attention  z_ax_sent : (B,S,D_AXIAL) ────────
...
#             # 6) broadcast back to tokens via linear bucket map i ↦ floor(i*k/S)
#             grid_size= 128
...
#         # ── 4) Confidence-curve encoder  (B,S,D_CONF) ───────────────────────
#         z_conf = self.conf_encoder(token_probs.to(conf_dtype),
#                                 mask.to(conf_dtype))                      # (B,S,D_CONF)

#         # ── 5) Concatenate all features and feed stop head ──────────────────
#         cat = torch.cat([ z_hid.to(out_dtype),
#                         # z_ax_tok.to(out_dtype),
#                         z_ax_sent.to(out_dtype),
#                         z_conf.to(out_dtype) ], dim=-1)                    # (B,S,C)

#         logits = self.stop_head(cat.view(B * S, -1))                        # (B·S,1)

#         # save_root = "/home/amirhosein/codes/SelfAwareMachine/visualization_attn"
...
#         #     B_, L_, S_, S2_ = A_sent.shape
#         #     assert S_ == S2_, f"expected square attention, got {S_}x{S2_}"
#         #     b = 0  # which batch item to visualize

#         #     with torch.no_grad():
#         #         for l in range(L_):
#         #             A = A_sent[min(b, B_-1), l].to(torch.float32).cpu().numpy()  # [S,S]
#         #             vmax = float(np.abs(A).max()) or 1.0

#         #             plt.figure(figsize=(6, 6))
#         #             plt.imshow(A, origin="lower", vmin=-vmax, vmax=vmax,
#         #                     interpolation="nearest", aspect="auto")
#         #             plt.colorbar(fraction=0.046, pad=0.04)
#         #             plt.title(f"Layer {l} • S={S_}")
#         #             out_path = os.path.join(save_root, f"attn_L{l:02d}_S{S_}.png")
...
#     #     else:
#     #         SEP_ID = 198                 
#     #         # 3a)  (B,C,S,S)  → (B,C,Ns,Ns)     *channels preserved*
...
#     #     # p = p.clamp_(eps, 1.0 - eps)               # 1️⃣ strictly inside (0,1)

#     #     log_surv = torch.cumsum(torch.log1p(-p) * mask, dim=1) - torch.log1p(-p) * mask
#     #     logq = torch.log(p) + log_surv             # log q_t
#     #     logq = logq - logq.max(dim=1, keepdim=True).values     # 2️⃣ subtract max
#     #     q = (logq.exp() * mask)                    # still finite
#     #     q = q / (q.sum(dim=1, keepdim=True) + eps)            # 3️⃣ normalise
