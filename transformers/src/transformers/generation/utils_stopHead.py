...
class GenerationMixin:
    ...
    def generate(
        ...
        auxiliary_control_head: Optional[bool] = True,
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result = self._sample(
                ...
                synced_gpus=synced_gpus,
                streamer=streamer,
                auxiliary_control_head=auxiliary_control_head,
                **model_kwargs,
            )
    ...
    def _contrastive_search(
        ...
        **model_kwargs,
        ...
        # #myedit
        # stop_token_id = getattr(self.config, "stop_token_id", 151668)
        # assert stop_token_id is not None, "config.stop_token_id must be set to the </think> id"
        # stop_threshold = getattr(self.config, "stop_threshold", 0.5)

        # # We don’t have “probs” for the prompt tokens; initialize them to 1.0 so they’re neutral.
        # token_probs_so_far = torch.ones_like(input_ids, dtype=torch.float32, device=device)
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if model_kwargs.get("past_key_values") is None or (
                ...
                # #myedit
                # apply_budget = prev_token_is_newline
                # model_inputs.update({"apply_budget": apply_budget})
                # model_inputs.update({"token_probs_so_far": token_probs_so_far})
    ...
    #     # --- myedit Stop-head runtime state (NEW) ---
    #     device = input_ids.device
    #     stop_token_id = getattr(self.config, "stop_token_id", None)
    #     assert stop_token_id is not None, "config.stop_token_id must be set to the </think> id"
    #     stop_threshold = getattr(self.config, "stop_threshold", 0.5)
    #     token_probs_so_far = torch.ones_like(input_ids, dtype=torch.float32, device=device)
    #     prev_token_is_newline = True
    #     stop_head_active = bool(auxiliary_control_head)
    ...
    #         # --- myedit: tell model to run the stop head on this step ---
    #         apply_budget = stop_head_active and prev_token_is_newline
    #         model_inputs.update({"apply_budget": apply_budget})
    #         model_inputs.update({"token_probs_so_far": token_probs_so_far})
    ...
    #         # --- myedit: consult stop head; if it fires, force "</think>" ---
    #         decided_stop = False
    #         # If the model returned stop_prob, consult it
    #         if apply_budget and hasattr(outputs, "stop_prob") and outputs.stop_prob is not None:
    #             stop_p = outputs.stop_prob.view(input_ids.size(0), -1)[:, -1]  # (B,)
    #             should_stop = (stop_p > stop_threshold)

    #             if should_stop.any():
    #                 fallback = next_token_logits.argmax(dim=-1)
    #                 next_tokens = torch.where(
    #                     should_stop,
    #                     torch.tensor(stop_token_id, device=input_ids.device, dtype=input_ids.dtype),
    #                     fallback,
    #                 )
    #                 # For logging/storage paths below, define a consistent "scores" tensor
    #                 next_token_scores = next_token_logits  # raw logits path when forced stop
    #                 decided_stop = True

    #         if not decided_stop:
    #             next_token_scores = logits_processor(input_ids, next_token_logits)
    #             probs = torch.softmax(next_token_scores, dim=-1)
    #             next_tokens = torch.multinomial(probs, 1).squeeze(1) if do_sample else probs.argmax(dim=-1)
                
    #         else:
    #             # If we forced stop, define probs from raw logits so we can compute picked prob
    #             probs = torch.softmax(next_token_logits, dim=-1)

    #         picked_probs = probs.gather(1, next_tokens.view(-1, 1)).squeeze(1)  # (B,)
    #         # print(probs)
    #         token_probs_so_far = torch.cat([token_probs_so_far, picked_probs.unsqueeze(1)], dim=1)
    ...
    #         stop_head_active = stop_head_active and not bool((next_tokens == stop_token_id).any())
    ...
    #     stop_token_id = getattr(self.config, "stop_token_id", 151668)
    #     assert stop_token_id is not None, "config.stop_token_id must be set to the </think> id"
    #     stop_threshold = float(getattr(self.config, "stop_threshold", 0.5))
    #     newline_token_ids = getattr(self.config, "newline_token_ids", 198)
    #     # self.newline_token_id = 198 # "\n"

    #     B, cur_len = input_ids.shape
    #     token_probs_so_far = torch.ones_like(input_ids, dtype=torch.float32, device=device)

    #     # Vectorized gating state
    #     stop_head_active = torch.full((B,), bool(auxiliary_control_head), device=device)
    #     prev_token_is_newline = torch.ones(B, dtype=torch.bool, device=device)  # true at start so we check once
    ...
    #         # --- myedit: tell model to run the stop head on this step ---
    #         # Compute a global boolean: do any sequences request the stop head this step?
    #         apply_budget_vec = stop_head_active & prev_token_is_newline & unfinished_sequences.bool()
    #         apply_budget = bool(apply_budget_vec.any().item())
    #         print(stop_head_active, prev_token_is_newline)
    ...
    #         # --- myedit: consult stop head; if it fires, force "</think>" ---
    #         decided_stop = False
    #         print(outputs.stop_prob)
    #         if apply_budget and hasattr(outputs, "stop_prob") and outputs.stop_prob is not None:
    #             # (B,1,1) -> (B,)
    #             stop_p = outputs.stop_prob.view(input_ids.size(0), -1)[:, -1].clamp(1e-6, 1-1e-6)
    #             print(stop_p)
    #             # Only consider active & unfinished sequences
    #             should_stop = (stop_p > stop_threshold) & apply_budget_vec & unfinished_sequences.bool()

    #             if should_stop.any():
    #                 fallback = next_token_logits.argmax(dim=-1)
    #                 force_ids = torch.full_like(fallback, stop_token_id)
    #                 next_tokens = torch.where(should_stop, force_ids, fallback)
    #                 next_token_scores = next_token_logits  # for logging
    #                 probs = torch.softmax(next_token_logits, dim=-1)  # for picked_probs below
    #                 decided_stop = True
    ...
    #                         if do_sample else probs.argmax(dim=-1))
    #         else:
    #             # probs already defined from raw logits in the forced path
    #             pass
    ...
    #         #myedit
    #         # if "\n" in self.tokenizer.decode(next_tokens) and apply_budget:
    #         if newline_token_ids in next_tokens and apply_budget:
    #             prev_token_is_newline = True
    #         else:
    #             prev_token_is_newline = False
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        r"""
        Generates sequences using multinomial sampling; integrates a vectorized stop head that can
        force an end token (</think>) per sequence when its probability crosses a threshold.
        """
        # --- configuration & return controls ---
        pad_token_id = generation_config._pad_token_tensor
        output_attentions     = generation_config.output_attentions
        output_hidden_states  = generation_config.output_hidden_states
        output_scores         = generation_config.output_scores
        ...
        # --- stop-head runtime state ---
        device = input_ids.device
        stop_token_id  = getattr(self.config, "stop_token_id", 151668)  # </think>
        stop_threshold = float(getattr(self.config, "stop_threshold", 0.5))
        assert stop_token_id is not None, "config.stop_token_id must be set to the </think> id"

        # normalize newline token(s) → 1D LongTensor on device
        newline_ids = getattr(self.config, "newline_token_ids", 198)
        tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-4B")


        # per-sequence activation vector (set aux_control_head=True to enable)
        stop_head_active = torch.full((batch_size,), bool(auxiliary_control_head), device=device)
        # sentence-start flag (True initially so we consult once after the prompt)
        prev_token_is_newline = torch.ones(batch_size, dtype=torch.bool, device=device)

        # keep per-step picked probabilities (start with ones for the prompt)
        token_probs_so_far = torch.ones_like(input_ids, dtype=torch.float32, device=device)
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # consult stop-head only:
            # - at sentence starts (prev_token_is_newline)
            # - for sequences still unfinished
            # - after total length > 50 tokens
            len_ok = (input_ids.size(1) > 400)
            apply_budget_vec = stop_head_active & prev_token_is_newline & unfinished_sequences.bool() & len_ok
            apply_budget = bool(apply_budget_vec.any().item())
            # pass runtime info for stop-head
            model_inputs.update({"apply_budget": apply_budget, "token_probs_so_far": token_probs_so_far})

            # honor output controls if requested
            if output_attentions:
            ...
            # last-step logits
            last_logits = outputs.logits[:, -1, :].to(copy=True, dtype=torch.float32, device=input_ids.device)
            processed_scores = logits_processor(input_ids, last_logits)        # (B, V)
            probs = torch.softmax(processed_scores, dim=-1)                    # (B, V)

            # sample or greedy
            if do_sample:
                sampled = torch.multinomial(probs, 1).squeeze(1)               # (B,)
            else:
                sampled = probs.argmax(dim=-1)                                 # (B,)

            # default next tokens
            next_tokens = sampled

            # --- consult stop head when active this step ---
            if apply_budget and hasattr(outputs, "stop_prob") and (outputs.stop_prob is not None):
                # expect (B,1,1) or (B,1) → (B,)
                stop_p = outputs.stop_prob.view(batch_size, -1)[:, -1].clamp_(1e-6, 1 - 1e-6)

                # DEBUG PRINTS: stop probabilities and partial decode (for batch 0)
                try:
                    print("[stop_head] probs:", stop_p.detach().cpu().tolist())
                except Exception:
                    pass
                try:
                    txt_so_far = tokenizer.decode(input_ids[0].tolist(), skip_special_tokens=False)
                    print("[gen so far b0]:", txt_so_far)
                except Exception:
                    pass

                should_stop = (stop_p > stop_threshold) & apply_budget_vec & unfinished_sequences.bool()
                if should_stop.any():
                    force_ids = torch.full_like(sampled, stop_token_id)
                    next_tokens = torch.where(should_stop, force_ids, sampled)
                    # deactivate for sequences that just emitted </think>
                    stop_head_active = stop_head_active & ~(next_tokens == stop_token_id)

            # record prob of the actual emitted token
            picked_probs = probs.gather(1, next_tokens.view(-1, 1)).squeeze(1)  # (B,)
            token_probs_so_far = torch.cat([token_probs_so_far, picked_probs.unsqueeze(1)], dim=1)
            ...
            # sentence-start flag for next step (vectorized newline check)
            
            if newline_ids in next_tokens:
                print(next_tokens)
                prev_token_is_newline = True
            else:
                prev_token_is_newline = False

            # finished sequences → pad next token (HF convention)
            if has_eos_stopping_criteria:
                next_tokens = next_tokens * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)
